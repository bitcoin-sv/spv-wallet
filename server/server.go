...
func SetupServerRoutes(appConfig *config.AppConfig, services *config.AppServices, engine *gin.Engine) {
	...
	newTransactionBasicRoutes, newTransactionAPIRoutes, _ := transactions.NewTransactionsHandler(appConfig, services)
	...
	routes := []interface{}{
		...
		newTransactionBasicRoutes,
		newTransactionAPIRoutes,
		// Utxo routes
		utxoAPIRoutes,
		// xPub routes
		xPubAPIRoutes,
