...
func TestUnfoldError(t *testing.T) {
	...
	t.Run("unfold single SPVError", func(t *testing.T) {
		err := models.SPVError{Code: "test-err", Message: "test error", StatusCode: 500}
		unfolded := UnfoldError(err)
		require.Equal(t, "[models.SPVError(500)] test error", unfolded)
	})
	...
	t.Run("unfold SPVError wrapped by pkg.Wrap and then wrapped into SPVError", func(t *testing.T) {
		err1 := models.SPVError{Code: "test-err1", Message: "test error1", StatusCode: 404}
		err3 := models.SPVError{Code: "test-err2", Message: "test error3", StatusCode: 500}
		err := err1.Wrap(pkgerrors.Wrap(err3, "test error2"))
		unfolded := UnfoldError(err)
		require.Equal(t, "[models.SPVError(404)] test error1 -> [*errors.withStack] test error2: test error3 -> [*errors.withMessage] -> [models.SPVError(500)]", unfolded)
	})
	t.Run("wrapping with fmt.Errorf", func(t *testing.T) {
		err1 := models.SPVError{Code: "test-err1", Message: "test error1", StatusCode: 500}
		err := fmt.Errorf("Some error is here %w - in the middle of a string", err1)
		unfolded := UnfoldError(err)
		require.Equal(t, "[*fmt.wrapError] Some error is here test error1 - in the middle of a string -> [models.SPVError(500)]", unfolded)
	})

	t.Run("joining errors", func(t *testing.T) {
		err1 := models.SPVError{Code: "test-err1", Message: "test error1", StatusCode: 500}
		err2 := models.SPVError{Code: "test-err2", Message: "test error2", StatusCode: 502}
		err3 := fmt.Errorf("test error3")
		err := err1.Wrap(errors.Join(err2, err3))
		unfolded := UnfoldError(err)
		require.Equal(t, "[models.SPVError(500)] test error1 -> [*errors.joinError] ([models.SPVError(502)] test error2 AND [*errors.errorString] test error3)", unfolded)
	})
}
