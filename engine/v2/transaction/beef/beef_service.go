...
type TxRepository interface {
	// QueryTransactionInputSources retrieves transaction query results for the provided source transaction IDs.
	// Returns a slice of TxQueryResult and an error if the query fails.
	QueryTransactionInputSources(ctx context.Context, sourceTXIDs ...string) (TxQueryResultSlice, error)
}
...
func (s *Service) PrepareBEEF(ctx context.Context, tx *sdk.Transaction) (string, error) {
	...
	txID := tx.TxID().String()
	txQueryResult, err := s.repository.QueryTransactionInputSources(ctx, sourceTxIDs...)
	if err != nil {
