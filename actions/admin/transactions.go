...
// getTxAdminByID fetches a transaction by id for admins
...
// @Security	x-auth-xpub
func getTxAdminByID(c *gin.Context, _ *reqctx.AdminContext) {
	logger := reqctx.Logger(c)
	id := c.Param("id")
...
// getTransactions will fetch a list of transactions filtered by metadata
// Search for transactions filtering by metadata godoc
// @Summary		Search for transactions
// @Description	Search for transactions
// @Tags		Admin
...
// @Security	x-auth-xpub
func getTransactions(c *gin.Context, _ *reqctx.AdminContext) {
	logger := reqctx.Logger(c)
